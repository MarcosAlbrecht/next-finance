...
export const generateAiReport = async ({
  ...
  const firstDayOfMonth = new Date(numericYear, numericMonth - 1, 1); // Primeiro dia do mês
  const lastDayOfMonth = new Date(numericYear, numericMonth, 0); // Último dia do mês
  const transactions = await db.transaction.findMany({
    where: {
      date: {
        gte: new Date(`${year}-${month}-${firstDayOfMonth}`),
        lt: new Date(`${year}-${month}-${lastDayOfMonth}`),
      },
    },
  });
